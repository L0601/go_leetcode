...
func solve(sum int64, lastIndex int, questions [][]int) int64 {
	if lastIndex > 0 {
		lastIndex += questions[lastIndex-1][1] + 1
	}

	for i := lastIndex; i < len(questions); i++ {
		tmp := solve(sum+int64(questions[i][0]), i+1, questions)
		if tmp > sum {
			sum = tmp
		}
	}
